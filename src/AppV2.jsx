...
const KEYWORDS_PROMPT = `You are a friendly problem diagnosis assistant. Given a problem and an optional narrowing context, generate exactly 5 diagnostic keywords as a JSON array.

Each object MUST have:
- "id": short snake_case string
- "label": a short keyword (1-2 words max)
- "tooltip": a one-line plain-language description (max 18 words, include a concrete example or number when possible)

Return ONLY a valid JSON array — no markdown fences, no commentary. Match the user's language.

Example:
[
  { "id": "network", "label": "Network", "tooltip": "Slow Wi-Fi or 4G can add 2–3s latency on every request." }
]`;

const DETAILS_PROMPT = `You explain a single diagnostic keyword in the context of a user's problem. Write a friendly, concrete explanation of ~200 words (between 180 and 220 words). Cover: what this typically looks like, what to inspect first, and how to confirm or rule it out. Use 2–3 short paragraphs. No bullet lists. Match the user's language.`;

const INSIGHT_PROMPT = `You are a problem diagnosis assistant. The user is exploring their problem on a diagnostic map. Given their path so far, the keyword they're investigating, and their free-text answer, provide a brief diagnostic summary (3–5 sentences) that:
1. Acknowledges what's been narrowed down
2. Interprets their answer in context
...
async function fetchKeywords(problem, pathLabels, rejectedLabels = []) {
  ...
  if (pathLabels.length === 0) {
    msg = `Problem: "${problem}"\nGenerate 5 top-level diagnostic keywords to start mapping this problem.`;
  } else {
    msg = `Original problem: "${problem}"
Narrowed down through: ${pathLabels.join(" → ")}
Generate 5 sub-keywords that further narrow the cause *within* "${pathLabels[pathLabels.length - 1]}". Stay inside that sub-area.`;
  }
  if (rejectedLabels.length > 0) {
    msg += `\n\nThe user already ruled out these keywords: ${rejectedLabels.join(", ")}. Propose 5 entirely different angles.`;
  }
...
async function fetchDetails(problem, pathLabels, keyword) {
  const msg = `Problem: "${problem}"
Path: ${pathLabels.length ? pathLabels.join(" → ") : "(top level)"}
Keyword: "${keyword.label}"
Short brief: "${keyword.tooltip || ""}"
...
async function fetchInsight(problem, pathLabels, keyword, answer) {
  const msg = `Problem: "${problem}"
Path: ${pathLabels.length ? pathLabels.join(" → ") : "(top level)"}
Keyword under investigation: "${keyword.label}" — ${keyword.tooltip || ""}
...
function ParentCard({ label, tooltip }) {
  return (
...
function ActiveCard({ kw, onYes, onNo, onDetails, onInsight }) {
  return (
    <motion.div
      layoutId={`kw-${kw.id}`}
    ...
    >
      ...
      >
        Is your problem about <strong>{kw.label}</strong>?
      </motion.p>
      <div className="active-card">
        ...
        >
          <h2 className="active-title">{kw.label}</h2>
          <p className="active-desc">{kw.tooltip}</p>
          <div className="active-actions">
            <button className="btn-yes" onClick={() => onYes(kw)}>Yes</button>
            <button className="btn-no" onClick={() => onNo(kw)}>No</button>
            <button className="btn-more" onClick={() => onDetails(kw)}>More details</button>
            <button className="btn-insight" aria-label="Free-text insight" title="Free-text insight" onClick={() => onInsight(kw)}>💬</button>
          </div>
        </motion.div>
      </div>
    </motion.div>
...
function LevelView({
  ...
  const { parentLabel, parentTooltip, keywords, currentIndex, rejectedIds, chosenId, isRoot } = level;
  ...
  const needsActiveArea = active || (isCurrent && loading) || chosen;
    ...
    <div className="level">
      <div className="level-parent">
        {isRoot
          ...
          : <ParentCard label={parentLabel} tooltip={parentTooltip} />}
      </div>

      {needsActiveArea || rejected.length > 0 ? <Connector /> : null}

      {(needsActiveArea || rejected.length > 0) && (
        <div className="level-row">
          ...
          <div className="active-area">
            <AnimatePresence mode="popLayout">
              ...
              {chosen && <ChosenBadge key={`chosen-${chosen.id}`} kw={chosen} />}
              {!active && !chosen && isCurrent && loading && (
                <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <LoadingBlock label="thinking of the next angle…" />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
...
function DetailsModal({ keyword, content, loading, onClose }) {
  return (
    <ModalShell onClose={onClose}>
      ...
      <h2 className="modal-title">{keyword.label}</h2>
...
export default function AppV2() {
  ...
  const handleYes = async (kw) => {
    ...
    try {
      ...
      const kws = await fetchKeywords(problem, newPath);
      setLevels((prev) => [...prev, {
  ...
  const handleUndo = () => {
    setLevels((prev) => {
      ...
      if (last.rejectedIds.length > 0) {
        const lastRejected = last.rejectedIds[last.rejectedIds.length - 1];
        last.rejectedIds = last.rejectedIds.slice(0, -1);
        last.currentIndex = Math.max(0, last.currentIndex - 1);
        const idx = last.keywords.findIndex((k) => k.id === lastRejected);
        if (idx > -1) {
          const arr = [...last.keywords];
          const [item] = arr.splice(idx, 1);
          arr.splice(last.currentIndex, 0, item);
...
.level-row{
  ...
  gap:24px;width:100%;min-height:220px;padding:8px 0 16px;flex-wrap:wrap;
}
.rejected-stack{
...
.active-wrap{display:flex;flex-direction:column;align-items:center;gap:12px;width:100%;max-width:420px}

.active-question{
  font-size:15px;color:#4b5165;text-align:center;line-height:1.5;
}
.active-question strong{color:#5b58f0;font-weight:700}
...
.active-title{
  ...
  color:#1f2330;margin-bottom:10px;letter-spacing:-0.02em;
}
.active-desc{font-size:14px;color:#4b5165;line-height:1.65}

.active-actions{display:flex;gap:8px;margin-top:20px;flex-wrap:wrap}
.btn-yes{
...
.btn-yes:hover{transform:translateY(-1px);box-shadow:0 8px 18px rgba(99,102,241,0.30)}
.btn-no{
  background:#ffffff;color:#6b7280;border:1px solid #e6e8ff;border-radius:11px;
  font-family:inherit;font-size:13px;font-weight:600;padding:11px 18px;cursor:pointer;
  transition:all .15s;flex:1;min-width:64px;
}
.btn-no:hover{color:#1f2330;border-color:#c7caff;background:#fbfbff}
.btn-more{
...
.btn-more:hover{background:#e1e4ff}
.btn-insight{
...
.btn-insight:hover{border-color:#c7caff;background:#fbfbff}

/* chosen badge (sits in the active spot once Yes is clicked) */
.chosen-badge{
...
.modal-title{
  font-family:'Plus Jakarta Sans',sans-serif;font-size:24px;font-weight:800;
  color:#1f2330;margin-bottom:6px;letter-spacing:-0.02em;
}
